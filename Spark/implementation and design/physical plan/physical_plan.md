...
- [Job 物理执行](#job-物理执行)
	- [一个复杂 job 的逻辑执行图](#一个复杂-job-的逻辑执行图)
	- [物理图的执行](#物理图的执行)
	- [生成 job](#生成-job)
	- [提交 job 的实现细节](#提交-job-的实现细节)
	- [Discussion](#discussion)
	- [ComplexJob 的源代码](#complexjob-的源代码)
...
## 一个复杂 job 的逻辑执行图
---
![logical_complex_job](logical_complex_job.png)
...
![physical_complex_job](physical_complex_job.png)
...
![logical_cartesian](logical_cartesian.png)

经过算法划分后结果如下：

![physical_cartesian](physical_cartesian.png)
...
>代码实现：每个 RDD 包含的 getDependency() 负责确立 RDD 的数据依赖，compute() 方法负责接收 parent RDDs 或者 data block 流入的 records，进行计算，然后输出 record。
